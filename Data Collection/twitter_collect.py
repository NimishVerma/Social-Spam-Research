...
def search(query, max_items=200, startdate=None, enddate=None, f=0):
    ...
    try:
        res = tweepy.Cursor(api.search, q=query, count=max_items, since=start_date, until=end_date, lang="en", tweet_mode="extended").items(max_items)
...
for i, keyword in enumerate(file):
    ...
    query = keyword
    search(query, max_items=int(130000/num_lines))
